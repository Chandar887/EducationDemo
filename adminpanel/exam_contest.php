...
if (isset($_GET['exam_contest_id']) && $_GET['exam_contest_id'] != "") {
    $contest_id = $_GET['exam_contest_id'];
}

$contstq = mysqli_query($db->con, "SELECT * FROM `exam_contest` where id = '$contest_id'");
?>
...
<!--                    <form method='get' action='<?php echo $currenturl; ?>' class="form-inline mr-4">
        ...
        <?php
        if (isset($request['exam_contest_id']) && $cData = $db->selectRow("exam_contest", "", "id = '{$request['exam_contest_id']}'")) {
//            $cData['category_name'] = "";
...
//            }
            ...
            <div class="row">
                <div class="col-md-4 col-12">
                    ...
                    <h6 class="h5 mb-2 text-muted"><?php echo $cData['category_name']; ?></h6>
...
<!--                        <div class="list-group-item flex-column align-items-start">
                        ...
                        <?php
                            $countques = mysqli_fetch_assoc(mysqli_query($db->con, "SELECT count(id) as countque FROM `contest_que` WHERE contest_id = '$contest_id'"));
                        ?>
                        <div class="list-group-item flex-column align-items-start">
                            <div class="d-flex w-100 justify-content-between">
                                <div class="mb-1">Total Questions</div>
                                <small><?php echo $countques['countque']; ?></small>
                            </div>
                        </div>
                        <div class="list-group-item flex-column align-items-start">
                            <div class="d-flex w-100 justify-content-between">
...
// echo "SELECT * FROM `quiz_score` WHERE contest_id = '$contest_id' AND created_at = '$created_at' ORDER BY quiz_score DESC,complete_time ASC";die;
                        ...
                        $rank_q = mysqli_query($db->con, "SELECT * FROM `quiz_play_detail` WHERE exam_id = '$contest_id' ORDER BY score DESC,complete_time ASC");
                                ...
                                while ($quiz_data = mysqli_fetch_array($rank_q)) {
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td class="text-right"><a href="view_quiz_details.php?user_id=<?php echo $user_id; ?>&&exam_id=<?php echo $contest_id; ?>" class="btn btn-sm btn-primary"><i class="fa fa-eye"></i></a></td>
